  ...
  create or replace view `sec-edgar-intelligence`.`sec_dbt_dev`.`stg_shares_outstanding`
  OPTIONS()
  as -- Concepts: CommonStockSharesOutstanding, WeightedAverageSharesBasic — quarterly instants.
...
with latest_fetch as (
    ...
    from `sec-edgar-intelligence`.`raw`.`raw_shares_outstanding`
