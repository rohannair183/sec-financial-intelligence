  ...
  create or replace view `sec-edgar-intelligence`.`sec_dbt_dev`.`stg_shares_eps_annual`
  OPTIONS()
  as -- Concepts: EarningsPerShareBasic, EarningsPerShareDiluted — annual, USD-per-shares.
...
with latest_fetch as (
    ...
    from `sec-edgar-intelligence`.`raw`.`raw_shares_and_eps_annual`
