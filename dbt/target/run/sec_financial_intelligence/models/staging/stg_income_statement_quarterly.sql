  ...
  create or replace view `sec-edgar-intelligence`.`sec_dbt_dev`.`stg_income_statement_quarterly`
  OPTIONS()
  as -- Concepts: Revenues, GrossProfit, OperatingIncome, NetIncome — quarterly durations.
...
with latest_fetch as (
    ...
    from `sec-edgar-intelligence`.`raw`.`raw_income_statement_quarterly`
