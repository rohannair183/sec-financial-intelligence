  ...
  create or replace view `sec-edgar-intelligence`.`sec_dbt_dev`.`stg_cash_flow_quarterly`
  OPTIONS()
  as -- Concepts: OperatingCF, CapEx — quarterly durations.
...
with latest_fetch as (
    ...
    from `sec-edgar-intelligence`.`raw`.`raw_cash_flow_quarterly`
